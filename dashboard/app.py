...
_build_status = {"running": False, "done": False, "error": None, "step": ""}
...
def _build_in_background():
    ...
    try:
        ...
        _build_status["step"] = "Downloading batting stats (2022-2024)..."
        frames = []
        ...
        _build_status["step"] = "Downloading pitching stats (2022-2024)..."
        frames = []
        for year in [2022, 2023, 2024]:
        ...
        _build_status["step"] = "Downloading Statcast 2024 — this takes ~4 minutes..."
        ...
        _build_status["step"] = "Building DuckDB warehouse..."
...
# ── FIRST-TIME BUILD SCREEN ───────────────────────────────────────────────────
conn = get_conn()

if conn is None:
    st.title("⚾ MLB Baseball Data Warehouse")
    st.markdown("---")
    st.info("🔧 **First-time setup:** Building data warehouse from MLB data sources...")

    if not _build_status["running"] and not _build_status["done"] and not _build_status["error"]:
        t = threading.Thread(target=_build_in_background, daemon=True)
        t.start()

    if _build_status["error"]:
        st.error(f"❌ Build failed: {_build_status['error']}")
        st.write("Please try restarting the app.")
    else:
        step = _build_status["step"] or "Starting..."
        st.write(f"**Status:** {step}")
        st.markdown("""
        **What's happening:**
        - Downloading batting & pitching stats from FanGraphs (2022–2024)
        - Downloading Statcast data from Baseball Savant (2024 season)
        - Building Bronze/Silver/Gold warehouse in DuckDB

        ⏱️ This takes **5–8 minutes** on first load. The page refreshes automatically every 5 seconds.
        """)
        progress_map = {
            "": 0,
            "Starting...": 2,
            "Downloading batting stats (2022-2024)...": 20,
            "Downloading pitching stats (2022-2024)...": 40,
            "Downloading Statcast 2024 — this takes ~4 minutes...": 60,
            "Building DuckDB warehouse...": 90,
            "Complete!": 100,
        }
        pct = progress_map.get(step, 50)
        st.progress(pct, text=step)
